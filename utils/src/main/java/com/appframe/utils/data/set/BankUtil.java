...
import com.appframe.utils.aoglrithm.SearchUtil;
...
public class BankUtil {
	...
	// 获取银行名称
    public static String getBankName(char[] charBin, int offset) {
        long longBin = 0;
        for (int i = 0; i < 6; i++) {
            longBin = (longBin * 10) + (charBin[i + offset] - 48);
        }
        Logger.getLogger().e("bankBin: " + longBin);
        int index = SearchUtil.binarySearch(bankBin, longBin);
        if (index == -1) {
            return "磁条卡卡号";
        }
